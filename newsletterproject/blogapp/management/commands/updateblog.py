...
class Command(BaseCommand):
    ...
    def update_or_create_post(self, existing_posts, slug, title, 
                              content, publish_date, tags_list, toc_list, subtitle, author
                            ):
        ...
        if matching_posts.exists():
            ...
            self.update_post(matching_posts.first(), title, content, 
                             publish_date, tags_list, toc_list, subtitle, author
                             )
        else:
            # Create new post
            self.create_post(title, content, publish_date, 
                             slug, tags_list, toc_list, subtitle, author
                             )

    def update_post(self, post, title, content, publish_date,
                    tags_list, toc_list, subtitle, author
                    ):
        if (
            post.title != title
            ...
            or post.subtitle != subtitle
            or post.author != author
        ):
            post.title = title
    ...
    def create_post(self, title, content, publish_date, slug, 
                    tags_list, toc_list, subtitle, author
                    ):
        new_post = BlogPost(
            title=title,
